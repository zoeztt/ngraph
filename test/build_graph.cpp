...
TEST(build_graph, build_simple)
{
    ...
    auto arg0 = make_shared<op::Parameter>(f32, Shape{7, 3});
    auto arg1 = make_shared<op::Parameter>(f32, Shape{3});
    auto arg2 = make_shared<op::Parameter>(f32, Shape{32, 7});
    auto arg3 = make_shared<op::Parameter>(f32, Shape{32, 7});
...
{
    auto arg0 = make_shared<op::Parameter>(f32, Shape{32, 3});
    auto arg1 = make_shared<op::Parameter>(f32, Shape{3});
    auto arg2 = make_shared<op::Parameter>(f32, Shape{32});

    auto dot = make_shared<op::Dot>(arg0, arg1);
    auto add = make_shared<op::Add>(dot, arg2);

    auto parg = make_shared<op::Parameter>(f32, Shape{});
...
TEST(build_graph, literal)
{
    ...
    auto float0 = make_shared<op::Constant>(f32, Shape{}, float_t);
    ASSERT_EQ(float0->get_vector<float>(), std::vector<float>{3.0});
    ASSERT_EQ(float0->get_element_type(), f32);
    ...
    auto int32_0 = make_shared<op::Constant>(i32, Shape{}, int32);
    ASSERT_EQ(int32_0->get_vector<int32_t>(), std::vector<int>{3});
    ASSERT_EQ(int32_0->get_element_type(), i32);
...
TEST(build_graph, tensor)
{
    ...
    auto int32_0 = make_shared<op::Constant>(i32, ishape, idata);
    ASSERT_EQ(int32_0->get_element_type(), i32);
