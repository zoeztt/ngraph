...
NGRAPH_TEST(${BACKEND_NAME}, argmin_trivial)
{
    ...
    auto f = make_shared<Function>(make_shared<op::ArgMin>(A, 0, i32), ParameterVector{A});
    ...
    auto result = backend->create_tensor(i32, rshape);
...
NGRAPH_TEST(${BACKEND_NAME}, argmin_4D_axis_3_i64)
{
    ...
    Shape rshape{2, 2, 5};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMin>(A, 3, i64), ParameterVector{A});
    auto backend = runtime::Backend::create("${BACKEND_NAME}");
    // Create some tensors for input/output
    auto a = backend->create_tensor(f32, shape);
    copy_data(a,
              test::NDArray<float, 4>({{{{0.5f, 1.5f, 0.8f, 2.9f, 1.05f}, // img 0 ch 0
                  ...
                  .get_vector());
    auto result = backend->create_tensor(i64, rshape);
    backend->call_with_validate(backend->compile(f), {result}, {a});
...
NGRAPH_TEST(${BACKEND_NAME}, argmin_4D_axis_3)
{
    ...
    Shape rshape{2, 2, 5};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMin>(A, 3, i32), ParameterVector{A});
    auto backend = runtime::Backend::create("${BACKEND_NAME}");
    // Create some tensors for input/output
    auto a = backend->create_tensor(f32, shape);
    copy_data(a,
...
NGRAPH_TEST(${BACKEND_NAME}, argmax_trivial)
{
    ...
    Shape rshape{3};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMax>(A, 0, i32), ParameterVector{A});
...
NGRAPH_TEST(${BACKEND_NAME}, argmax_3D_axis_1) // Along Height
{
    ...
    Shape rshape{3, 2};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMax>(A, 1, i32), ParameterVector{A});
...
NGRAPH_TEST(${BACKEND_NAME}, argmax_3D_axis_2) // Along Width
{
    ...
    Shape rshape{3, 4};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMax>(A, 2, i32), ParameterVector{A});
...
NGRAPH_TEST(${BACKEND_NAME}, argmax_4D_axis_3)
{
    ...
    Shape rshape{2, 2, 5};
    auto A = make_shared<op::Parameter>(f32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMax>(A, 3, i32), ParameterVector{A});
...
NGRAPH_TEST(${BACKEND_NAME}, argmax_4D_axis_3_i64_in_i32)
{
    ...
    Shape rshape{2, 2, 5};
    auto A = make_shared<op::Parameter>(i32, shape);
    auto f = make_shared<Function>(make_shared<op::ArgMax>(A, 3, i64), ParameterVector{A});
    auto backend = runtime::Backend::create("${BACKEND_NAME}");
    // Create some tensors for input/output
    auto a = backend->create_tensor(i32, shape);
    copy_data(a,
