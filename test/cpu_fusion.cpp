...
TEST (cpu_fusion, bn_bprop2)
{
    ...
    vector<float> dataInput {		
			...
			10.76331902, 11.51178265, 10.31018162, 12.2993021,  14.17626667,
			14.63498497, 13.63494492, 13.84248161, 11.34602547, 13.22014618,
			10.46686649, 10.39842987, 12.94806862, 11.71670246, 14.94438076,
			13.13236618, 13.40889645, 12.76128387, 11.34430027, 11.86629677,
			11.11464024, 10.93221283, 11.95324039, 10.96581173, 13.05455494,
			14.41404247, 13.11169434, 11.26559448, 10.89965153, 14.08202171,
			11.12685776, 12.58428574, 12.59247875, 13.00187492, 12.66310215,
			10.06655025, 12.62048626, 14.47942352, 13.84950638, 10.61425877,
			11.47936344, 13.06011772, 13.63069057, 12.31748772, 13.84555244,
			10.95815468, 12.78933334, 12.75389099
    ...
    copy_data(_input, dataInput);
    auto _mean = backend->make_primary_tensor_view(element::f32, mean_shape);
    //copy_data(_mean, vector<float>{12.564729, 12.803121, 11.816769});
    copy_data(_mean, vector<float>{12.56472874, 12.80312157, 11.81676865});
    auto _var = backend->make_primary_tensor_view(element::f32, var_shape);
    //copy_data(_var, vector<float>{1.9455765, 1.3277245, 1.2816358});
    copy_data(_var, vector<float>{1.94557643,  1.32772446,  1.28163588});
