        ...
        namespace cpu
        {
            namespace kernel
            {
                             ...
                             const Shape& output_shape)
                {
                    ...
                    //Eigen::Tensor<ElementType, OutRank>::Dimensions f;
                    ...
                    Eigen::TensorMap<Eigen::Tensor<ElementType, OutRank, Eigen::RowMajor>> out(
                        output, out_dims);
                    Eigen::TensorMap<Eigen::Tensor<ElementType, InRank, Eigen::RowMajor>> in(input,
                                                                                             in_dims);

                    out.device(eigen::global_thread_pool_device) = in.shuffle(axis_order).reshape(out_dims);
                }
            }
        }
    }
