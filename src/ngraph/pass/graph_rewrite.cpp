...
void ngraph::pass::CPUFusion::construct_identity_reshape_pattern()
{
	...
	auto callback = [op](pattern::Matcher& m) {
		NGRAPH_DEBUG << "In callback for construct_identity_reshape_pattern against node = " << m.match_root()->get_name();
